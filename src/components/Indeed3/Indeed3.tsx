...
type Props = {
  isdisplayNewText: boolean;
  jobName: string;
  companyName: string;
  workPlace: string;
  tagList1: string[];
  tagList2: { name: string; imgLogo: string }[];
  application: string;
...
export const Indeed3: FC<Props> = ({
  ...
  return (
    <div className={styles.container}>
      {isdisplayNewText && <div className={styles.container__newText}>新着</div>}
      ...
      <div className={styles.container__application}>
        <div className={styles.container__applicationTag}>
          <span className={styles.container__application__applicationIcon}></span>
          {application}
        </div>
        {isdisplayResponseTag && (
          <div className={styles.container__responseTag}>
            <span className={styles.container__application__responseIcon}></span>返信率の高い企業
          </div>
        )}
      </div>
      <div className={styles.container__descripution}>{descripution}</div>
      <div className={styles.container__recruitingDate}>{recruitingDate}</div>
    </div>
  );
};
